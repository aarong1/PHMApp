...
<hr>
  ...
  div(id = 'target_echart',class = 'd-flex align-items-center justify-content-center',
      data.frame(year = 2023, incidence = 1, year=2023) |>
      ...
      for (time in 1:test_specification$model$duration){
        ...
        if (FALSE){  
          ...
          df <- past_populations |>
            ...
            filter(.data$stroke == TRUE) |>
            mutate(n = .data$n * test_specification$population$scale_down_factor)
          print(df)
          ...
          series_list <- df %>%
            group_by(.data$run) %>%
            summarise(data = list(map2(as.character(.data$year), .data$n, ~ list(.x, .y)))) %>%
            mutate(name = paste0("run ", .data$run)) %>%
            transmute(name, data) %>%
