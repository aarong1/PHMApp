...
absolute_ethnicity_facet_plot_apex <- eth_absolute |> 
  ...
  apexcharter::apex(mapping = aes( region, Count,fill = Ethnicity),
                    type = "col") |> #`Age Group`,
    ax_tooltip() #|> 
  #apexcharter::ax_chart() |>  
  #ax_facet_wrap(ncol = 2, facets = vars(region)) 
...
# Load GeoJSON
soa <- sf::read_sf('./data/soa_shapefiles')
#soa <- sf::read_sf('soa.geojson')

soa <- st_transform(soa, 'WGS84')

soa <- st_make_valid(soa)

object.size(soa)

soa <- st_simplify(soa,TRUE,100)
